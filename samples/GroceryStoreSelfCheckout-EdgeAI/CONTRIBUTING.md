...
- A compatible YOLO26 ONNX model copied to `EdgeAIKiosk1\Models\`
...
dotnet build EdgeAIKiosk1.slnx -r win-arm64
```

If your SDK does not support `.slnx`, build the project file directly:

```powershell
dotnet build EdgeAIKiosk1\EdgeAIKiosk1.csproj -r win-arm64
```
